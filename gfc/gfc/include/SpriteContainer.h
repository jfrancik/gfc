...
namespace std
{
	...
	public:
		...
		void delete_if(_Pr pr)
		{
			[this](auto _pr)
			{
				if constexpr (is_base_of<list<_Ty::value_type>, _Ty>::value)
					remove_if(_pr);
				else
					erase(std::remove_if(begin(), end(), _pr), end());
			}([pr](_Ty::value_type t) { bool b = pr(t); if constexpr(std::is_pointer<_Ty::value_type>::value) if (b) delete t; return b; });
		}
		...
		void delete_all() { for_each([](_Ty::value_type t) { if constexpr(std::is_pointer<_Ty::value_type>::value) delete t; }); clear(); }
	};
}
